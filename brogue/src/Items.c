...
item *makeItemInto(item *theItem, unsigned long itemCategory, short itemKind)
{
  ...
  case WEAPON:
    ...
    if (rand_percent(40))
    {
      ...
      else if (rand_range(3, 10) * ((theItem->flags & ITEM_ATTACKS_HIT_SLOWLY) ? 2 : 1) / ((theItem->flags & ITEM_ATTACKS_QUICKLY) ? 2 : 1) > theItem->damage.lowerBound)
      {
...
item *placeItem(item *theItem, short x, short y)
{
  ...
  if (cellHasTerrainFlag(x, y, T_IS_DF_TRAP) && !cellHasTerrainFlag(x, y, T_MOVES_ITEMS) && !(pmap[x][y].flags & PRESSURE_PLATE_DEPRESSED))
  {
...
void fillItemSpawnHeatMap(unsigned short heatMap[DCOLS][DROWS], unsigned short heatLevel, short x, short y)
{
  ...
  for (dir = 0; dir < 4; dir++)
  {
    newX = x + nbDirs[dir][0];
    newY = y + nbDirs[dir][1];
    if (coordinatesAreInMap(newX, newY) && (!cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY | T_IS_DEEP_WATER | T_LAVA_INSTA_DEATH | T_AUTO_DESCENT) || cellHasTMFlag(newX, newY, TM_IS_SECRET)) && heatLevel < heatMap[newX][newY])
    {
...
void populateItems(short upstairsX, short upstairsY)
{
  ...
  for (j = 0; j < DROWS; j++)
  {
    for (i = 0; i < DCOLS; i++)
    {
      ...
      if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_ITEMS | T_PATHING_BLOCKER) || (pmap[i][j].flags & (IS_CHOKEPOINT | IN_LOOP | IS_IN_MACHINE)) || passableArcCount(i, j) > 1)
  ...
  for (i = 0; i < numberOfItems; i++)
  {
    ...
    if ((rogue.foodSpawned + foodTable[RATION].strengthRequired / 3) * 4 <= (pow(rogue.depthLevel, 1.35) + randomDepthOffset) * foodTable[RATION].strengthRequired * 0.45 + FLOAT_FUDGE)
    {
...
void pickUpItemAt(short x, short y)
{
  ...
  if ((theItem->flags & ITEM_KIND_AUTO_ID) && tableForItemCategory(theItem->category, NULL) && !(tableForItemCategory(theItem->category, NULL)[theItem->kind].identified))
  {

    identifyItemKind(theItem);
  }

  if ((theItem->category & WAND) && wandTable[theItem->kind].identified && wandTable[theItem->kind].range.lowerBound == wandTable[theItem->kind].range.upperBound)
  {
...
void conflateItemCharacteristics(item *newItem, item *oldItem)
{
  ...
  newItem->flags |= (oldItem->flags & (ITEM_MAGIC_DETECTED | ITEM_IDENTIFIED | ITEM_PROTECTED | ITEM_RUNIC | ITEM_RUNIC_HINTED | ITEM_CAN_BE_IDENTIFIED | ITEM_MAX_CHARGES_KNOWN));
...
item *addItemToPack(item *theItem)
{
  ...
  else if (theItem->category & WEAPON && theItem->quiverNumber > 0)
  {
    for (tempItem = packItems->nextItem; tempItem != NULL; tempItem = tempItem->nextItem)
    {
      if (theItem->category == tempItem->category && theItem->kind == tempItem->kind && theItem->quiverNumber == tempItem->quiverNumber)
      {
        // We found a match!
        stackItems(tempItem, theItem);
...
void swapItemToEnchantLevel(item *theItem, short newEnchant, boolean enchantmentKnown)
{
  ...
  if (((theItem->category & STAFF) && (newEnchant < 2)) || ((theItem->category & CHARM) && (newEnchant < 1)) || ((theItem->category & WAND) && (newEnchant < 0)))
  {
...
void updateFloorItems()
{
  ...
  for (theItem = floorItems->nextItem; theItem != NULL; theItem = nextItem)
  {
    ...
    x = theItem->xLoc;
    y = theItem->yLoc;
    if ((cellHasTerrainFlag(x, y, T_IS_FIRE) && (theItem->flags & ITEM_FLAMMABLE)) || (cellHasTerrainFlag(x, y, T_LAVA_INSTA_DEATH) && !(theItem->category & AMULET)))
    {
    ...
    if (pmap[x][y].machineNumber && pmap[x][y].machineNumber == pmap[player.xLoc][player.yLoc].machineNumber && (theItem->flags & ITEM_KIND_AUTO_ID))
    {
    ...
    if (cellHasTMFlag(x, y, TM_SWAP_ENCHANTS_ACTIVATION) && pmap[x][y].machineNumber)
    {
      ...
      while (nextItem != NULL && pmap[x][y].machineNumber == pmap[nextItem->xLoc][nextItem->yLoc].machineNumber && cellHasTMFlag(nextItem->xLoc, nextItem->yLoc, TM_SWAP_ENCHANTS_ACTIVATION))
      {
...
boolean itemCanBeCalled(item *theItem)
{
  ...
  else if ((theItem->category & (POTION | SCROLL)) && !tableForItemCategory(theItem->category, NULL)[theItem->kind].identified)
  {
    return true;
  }
  else
...
void call(item *theItem)
{
  ...
  if (theItem == NULL)
  {
    ...
    for (tempItem = packItems->nextItem; tempItem != NULL; tempItem = tempItem->nextItem)
    {
      if ((tempItem->category & (POTION | SCROLL)) && tableForItemCategory(tempItem->category, NULL)[tempItem->kind].identified)
      {
  ...
  if (tableForItemCategory(theItem->category, NULL) && !(tableForItemCategory(theItem->category, NULL)[theItem->kind].identified))
  {
...
void itemDetails(char *buf, item *theItem)
{
  ...
  // introductory text
  if (tableForItemCategory(theItem->category, NULL) && (tableForItemCategory(theItem->category, NULL)[theItem->kind].identified || rogue.playbackOmniscience))
  {
  ...
  case ARMOR:
    ...
    if ((theItem->category & ARMOR) && !(theItem->flags & ITEM_EQUIPPED) && (current != armorAggroAdjustment(theItem)))
    {
        ...
        if ((theItem->flags & ITEM_RUNIC_IDENTIFIED) || rogue.playbackOmniscience)
        {
          ...
          else
          {
            ...
            if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience)
            {
              ...
              if (((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) && runicWeaponChance(theItem, false, 0) < runicWeaponChance(theItem, true, (float)(enchant + enchantIncrement(theItem))))
              {
  ...
  case RING:
    ...
    else
    {
      ...
      if ((theItem->charges < RING_DELAY_TO_AUTO_ID || (theItem->flags & (ITEM_MAGIC_DETECTED | ITEM_IDENTIFIED))) && theItem->enchant1 > 0)
                  ...
                  const short maxDistance, const boolean returnLastEmptySpace)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (monst && !monsterIsHidden(monst, monsterAtLoc(originLoc[0], originLoc[1])) && !(monst->bookkeepingFlags & MB_SUBMERGED))
    {
...
boolean impermissibleKinkBetween(short x1, short y1, short x2, short y2)
{
  ...
  if (!cellHasTerrainFlag(x2, y1, T_OBSTRUCTS_DIAGONAL_MOVEMENT) && !cellHasTerrainFlag(x1, y2, T_OBSTRUCTS_DIAGONAL_MOVEMENT))
  {
...
boolean polymorph(creature *monst)
{
  ...
  if (((monst->creatureState == MONSTER_FLEEING) && (monst->info.flags & (MONST_MAINTAINS_DISTANCE | MONST_FLEES_NEAR_DEATH))) || (monst->info.abilityFlags & MA_HIT_STEAL_FLEE))
  {
...
void negationBlast(const char *emitterName, const short distance)
{
  ...
  for (monst = monsters->nextCreature; monst != NULL;)
  {
    ...
    if ((pmap[monst->xLoc][monst->yLoc].flags & IN_FIELD_OF_VIEW) && (player.xLoc - monst->xLoc) * (player.xLoc - monst->xLoc) + (player.yLoc - monst->yLoc) * (player.yLoc - monst->yLoc) <= distance * distance)
    {
  ...
  {
    if ((pmap[theItem->xLoc][theItem->yLoc].flags & IN_FIELD_OF_VIEW) && (player.xLoc - theItem->xLoc) * (player.xLoc - theItem->xLoc) + (player.yLoc - theItem->yLoc) * (player.yLoc - theItem->yLoc) <= distance * distance)
    {
...
void crystalize(short radius)
{
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if ((player.xLoc - i) * (player.xLoc - i) + (player.yLoc - j) * (player.yLoc - j) <= radius * radius && !(pmap[i][j].flags & IMPREGNABLE))
      {
...
boolean projectileReflects(creature *attacker, creature *defender)
{
  ...
  if (defender == &player && rogue.armor && (rogue.armor->flags & ITEM_RUNIC) && rogue.armor->enchant2 == A_IMMUNITY && monsterIsInClass(attacker, rogue.armor->vorpalEnemy) && monstersAreEnemies(attacker, defender))
  {
...
short reflectBolt(short targetX, short targetY, short listOfCoordinates[][2], short kinkCell, boolean retracePath)
{
  ...
  needRandomTarget = (targetX < 0 || targetY < 0 || (targetX == listOfCoordinates[kinkCell][0] && targetY == listOfCoordinates[kinkCell][1]));
                   ...
                   boolean *autoID, boolean *lightingChanged)
{
  ...
  if (monst && !(monst->bookkeepingFlags & MB_SUBMERGED))
  {
    ...
    case BE_TELEPORT:
      ...
      break;
    case BE_BECKONING:
      if (!(monst->info.flags & MONST_IMMOBILE) && caster && distanceBetween(caster->xLoc, caster->yLoc, monst->xLoc, monst->yLoc) > 1)
      {
...
boolean zap(short originLoc[2], short targetLoc[2], bolt *theBolt, boolean hideDetails)
{
  ...
  {
    if (cellHasTerrainFlag(listOfCoordinates[0][0], listOfCoordinates[0][1], (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION)) || ((pmap[listOfCoordinates[0][0]][listOfCoordinates[0][1]].flags & (HAS_PLAYER | HAS_MONSTER)) && !(monsterAtLoc(listOfCoordinates[0][0], listOfCoordinates[0][1])->bookkeepingFlags & MB_SUBMERGED)))
    {
  ...
  for (i = 0; i < numCells; i++)
  {
    ...
    if (monst && !(theBolt->flags & BF_NEVER_REFLECTS) && projectileReflects(shootingMonst, monst) && i < MAX_BOLT_LENGTH - max(DCOLS, DROWS))
    {
      ...
      if (monst == &player && rogue.armor && rogue.armor->enchant2 == A_REFLECTION && !(rogue.armor->flags & ITEM_RUNIC_IDENTIFIED))
      {
    ...
    if (cellHasTerrainFlag(x, y, (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION)) && theBolt->boltEffect == BE_TUNNELING && tunnelize(x, y))
    {
    ...
    if (i + 1 < numCells && !(theBolt->flags & BF_NEVER_REFLECTS))
    {
      ...
      x2 = listOfCoordinates[i + 1][0];
      y2 = listOfCoordinates[i + 1][1];
      if (cellHasTerrainFlag(x2, y2, (T_OBSTRUCTS_VISION | T_OBSTRUCTS_PASSABILITY)) && (projectileReflects(shootingMonst, NULL) || cellHasTMFlag(x2, y2, TM_REFLECTS_BOLTS) || (theBolt->boltEffect == BE_TUNNELING && (pmap[x2][y2].flags & IMPREGNABLE))) && i < MAX_BOLT_LENGTH - max(DCOLS, DROWS))
      {
                        ...
                        boolean reverseDirection)
{
  ...
  for (i = 0; i < ROWS; i++)
  {
    ...
    {
      if (targetCount == 0 || deduplicatedTargetList[targetCount - 1][0] != rogue.sidebarLocationList[i][0] || deduplicatedTargetList[targetCount - 1][1] != rogue.sidebarLocationList[i][1])
      {
  ...
  for (i = reverseDirection ? targetCount - 1 : 0; reverseDirection ? i >= 0 : i < targetCount; reverseDirection ? i-- : i++)
  {
    ...
    if ((newX != player.xLoc || newY != player.yLoc) && (newX != targetX || newY != targetY) && (!requireOpenPath || openPathBetween(player.xLoc, player.yLoc, newX, newY)))
    {
...
short hiliteTrajectory(short coordinateList[DCOLS][2], short numCells, boolean eraseHiliting, boolean passThroughMonsters, const color *hiliteColor)
{
  ...
  for (i = 0; i < numCells; i++)
  {
    ...
    else if (!passThroughMonsters && pmap[x][y].flags & (HAS_MONSTER) && (playerCanSee(x, y) || player.status[STATUS_TELEPATHIC]))
    {
      monst = monsterAtLoc(x, y);
                   ...
                   boolean targetCanLeaveMap)
{
  ...
  *targetConfirmed = *canceled = *tabKey = false;
  ...
  do
  {
    ...
    {
      if (theEvent.param1 >= 0 && theEvent.param1 < mapToWindowX(0) && theEvent.param2 >= 0 && theEvent.param2 < ROWS - 1 && rogue.sidebarLocationList[theEvent.param2][0] > -1)
      {
      ...
      else if (coordinatesAreInMap(windowToMapX(theEvent.param1), windowToMapY(theEvent.param2)) || (targetCanLeaveMap && (theEvent.eventType != MOUSE_UP)))
      {

        // If the cursor is in the map area, or is allowed to leave the map and it isn't a click.
        if (theEvent.eventType == MOUSE_UP && !theEvent.shiftKey && (theEvent.controlKey || (cursor[0] == windowToMapX(theEvent.param1) && cursor[1] == windowToMapY(theEvent.param2))))
        {
    ...
    if (sidebarHighlighted && (!(pmap[cursor[0]][cursor[1]].flags & (HAS_PLAYER | HAS_MONSTER)) || !canSeeMonster(monsterAtLoc(cursor[0], cursor[1]))) && (!(pmap[cursor[0]][cursor[1]].flags & HAS_ITEM) || !playerCanSeeOrSense(cursor[0], cursor[1])) && (!cellHasTMFlag(cursor[0], cursor[1], TM_LIST_IN_SIDEBAR) || !playerCanSeeOrSense(cursor[0], cursor[1])))
    {
                     ...
                     const color *trajectoryColor)
{
  ...
  if (autoTarget)
  {
    if (rogue.lastTarget && canSeeMonster(rogue.lastTarget) && (targetAllies == (rogue.lastTarget->creatureState == MONSTER_ALLY)) && rogue.lastTarget->depth == rogue.depthLevel && !(rogue.lastTarget->bookkeepingFlags & MB_IS_DYING) && openPathBetween(player.xLoc, player.yLoc, rogue.lastTarget->xLoc, rogue.lastTarget->yLoc))
    {
  ...
  do
  {
    ...
    else if ((playerCanSeeOrSense(targetLoc[0], targetLoc[1]) && (pmap[targetLoc[0]][targetLoc[1]].flags & HAS_ITEM)) || cellHasTMFlag(targetLoc[0], targetLoc[1], TM_LIST_IN_SIDEBAR))
    {
      focusedOnSomething = true;
    }
...
void identifyItemKind(item *theItem)
{
  ...
  if (theTable)
  {
    ...
    if ((theItem->category & WAND) && theTable[theItem->kind].range.lowerBound == theTable[theItem->kind].range.upperBound)
    {
...
void autoIdentify(item *theItem)
{
  ...
  if ((theItem->category & (WEAPON | ARMOR)) && (theItem->flags & ITEM_RUNIC) && !(theItem->flags & ITEM_RUNIC_IDENTIFIED))
  {
...
boolean hitMonsterWithProjectileWeapon(creature *thrower, creature *monst, item *theItem)
{
  ...
  if (monst != &player && monst->creatureMode != MODE_PERM_FLEEING && (monst->creatureState != MONSTER_FLEEING || monst->status[STATUS_MAGICAL_FEAR]) && !(monst->bookkeepingFlags & MB_CAPTIVE) && monst->creatureState != MONSTER_ALLY)
  {
  ...
  if (thrownWeaponHit)
  {
    damage = (monst->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE) ? 0 : randClump(theItem->damage)) * pow(WEAPON_ENCHANT_DAMAGE_FACTOR, netEnchant(theItem));
...
void throwItem(item *theItem, creature *thrower, short targetLoc[2], short maxDistance)
{
  ...
  for (i = 0; i < numCells && i < maxDistance; i++)
  {
    ...
    if (pmap[x][y].flags & (HAS_MONSTER | HAS_PLAYER))
    {
      ...
      if (!(monst->bookkeepingFlags & MB_SUBMERGED))
      {
        ...
        //				continue;
        //			}
        if ((theItem->category & WEAPON) && theItem->kind != INCENDIARY_DART && hitMonsterWithProjectileWeapon(thrower, monst, theItem))
        {
          return;
        }
        break;
    ...
    if (cellHasTerrainFlag(x, y, (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION)))
    {
      if ((theItem->category & WEAPON) && (theItem->kind == INCENDIARY_DART) && (cellHasTerrainFlag(x, y, T_IS_FLAMMABLE) || (pmap[x][y].flags & (HAS_MONSTER | HAS_PLAYER))))
      {
      ...
      else if (cellHasTerrainFlag(x, y, T_OBSTRUCTS_PASSABILITY) && cellHasTMFlag(x, y, TM_PROMOTES_ON_PLAYER_ENTRY) && tileCatalog[pmap[x][y].layers[layerWithTMFlag(x, y, TM_PROMOTES_ON_PLAYER_ENTRY)]].flags & T_OBSTRUCTS_PASSABILITY)
      {
  ...
  {
    if (theItem->kind == POTION_CONFUSION || theItem->kind == POTION_POISON || theItem->kind == POTION_PARALYSIS || theItem->kind == POTION_INCINERATION || theItem->kind == POTION_DARKNESS || theItem->kind == POTION_LICHEN || theItem->kind == POTION_DESCENT)
    {
      switch (theItem->kind)
      {
      case POTION_POISON:
...
boolean playerCancelsBlinking(const short originLoc[2], const short targetLoc[2], const short maxDistance)
{
  ...
  if (maxDistance > 0)
  {
    if ((pmap[impactLoc[0]][impactLoc[1]].flags & DISCOVERED) && (tFlags & T_LAVA_INSTA_DEATH) && !(tFlags & T_ENTANGLES))
    {
...
boolean useStaffOrWand(item *theItem, boolean *commandsRecorded)
{
  ...
  boltKnown = (((theItem->category & WAND) && wandTable[theItem->kind].identified) || ((theItem->category & STAFF) && staffTable[theItem->kind].identified));
  if (!boltKnown)
  {
  ...
  if (confirmedTarget && boltKnown && theBolt.boltEffect == BE_BLINKING && playerCancelsBlinking(originLoc, zapTarget, maxDistance))
  {
...
void apply(item *theItem, boolean recordCommands)
{
  ...
  if ((theItem->category == SCROLL || theItem->category == POTION) && magicCharDiscoverySuffix(theItem->category, theItem->kind) == -1 && ((theItem->flags & ITEM_MAGIC_DETECTED) || tableForItemCategory(theItem->category, NULL)[theItem->kind].identified))
  {
...
void updateIdentifiableItem(item *theItem)
{
  ...
  else if ((theItem->category & (RING | STAFF | WAND)) && (theItem->flags & ITEM_IDENTIFIED) && tableForItemCategory(theItem->category, NULL)[theItem->kind].identified)
  {

    theItem->flags &= ~ITEM_CAN_BE_IDENTIFIED;
  }
  else if ((theItem->category & (WEAPON | ARMOR)) && (theItem->flags & ITEM_IDENTIFIED) && (!(theItem->flags & ITEM_RUNIC) || (theItem->flags & ITEM_RUNIC_IDENTIFIED)))
  {
...
unsigned long itemValue(item *theItem)
{
  ...
  case FOOD:
    ...
    break;
  case WEAPON:
    value = (signed long)(weaponTable[theItem->kind].marketValue * theItem->quantity * (1 + 0.15 * (theItem->enchant1 + (theItem->flags & ITEM_PROTECTED ? 1 : 0))));
    if (theItem->flags & ITEM_RUNIC)
    {
    ...
    break;
  case ARMOR:
    value = (signed long)(armorTable[theItem->kind].marketValue * theItem->quantity * (1 + 0.15 * (theItem->enchant1 + ((theItem->flags & ITEM_PROTECTED) ? 1 : 0))));
    if (theItem->flags & ITEM_RUNIC)
    {
