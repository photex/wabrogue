...
void mainInputLoop()
{
	...
	{ // repeats until the game ends
		...
		if (!playingBack && player.xLoc == cursor[0] && player.yLoc == cursor[1] && oldTargetLoc[0] == cursor[0] && oldTargetLoc[1] == cursor[1])
		{
		...
		do
		{
			...
			if (!playingBack)
			{
				...
				if (coordinatesAreInMap(cursor[0], cursor[1]))
				{
					...
					if (playerPathingMap[player.xLoc][player.yLoc] != 1 || pathDestination[0] != cursor[0] || pathDestination[1] != cursor[1])
					{
			...
			if (coordinatesAreInMap(cursor[0], cursor[1]))
			{
				hiliteCell(cursor[0],
						   cursor[1],
						   &white,
						   (steps <= 0 || (path[steps - 1][0] == cursor[0] && path[steps - 1][1] == cursor[1]) || (!playingBack && distanceBetween(player.xLoc, player.yLoc, cursor[0], cursor[1]) <= 1) ? 100 : 25),
						   true);
			...
			{
				if (((theEvent.param1 == ASCEND_KEY)  && (cursor[0] == rogue.upLoc[0])   && (cursor[1] == rogue.upLoc[1])) ||
				...
				if (originLoc[0] == cursor[0] && originLoc[1] == cursor[1])
				{
					...
					confirmMessages();
						 ...
						 || (distanceBetween(player.xLoc, player.yLoc, cursor[0], cursor[1]) == 1																																									  // includes diagonals
							 && (!diagonalBlocked(player.xLoc, player.yLoc, cursor[0], cursor[1], !rogue.playbackOmniscience) || ((pmap[cursor[0]][cursor[1]].flags & HAS_MONSTER) && (monsterAtLoc(cursor[0], cursor[1])->info.flags & MONST_ATTACKABLE_THRU_WALLS)) // there's a turret there
								 || ((terrainFlags(cursor[0], cursor[1]) & T_OBSTRUCTS_PASSABILITY) && (terrainMechFlags(cursor[0], cursor[1]) & TM_PROMOTES_ON_PLAYER_ENTRY)))))
...
void shuffleTerrainColors(short percentOfCells, boolean refreshCells)
{
	...
	for (i = 0; i < DCOLS; i++)
	{
		for (j = 0; j < DROWS; j++)
		{
			if (playerCanSeeOrSense(i, j) && (!rogue.automationActive || !(rogue.playerTurnNumber % 5)) && ((pmap[i][j].flags & TERRAIN_COLORS_DANCING) || (player.status[STATUS_HALLUCINATING] && playerCanDirectlySee(i, j))) && (i != rogue.cursorLoc[0] || j != rogue.cursorLoc[1]) && (percentOfCells >= 100 || rand_range(1, 100) <= percentOfCells))
			{
...
void getCellAppearance(short x, short y, uchar *returnChar, color *returnForeColor, color *returnBackColor)
{
	...
	if (monst)
	{
		monsterWithDetectedItem = (monst->carriedItem && (monst->carriedItem->flags & ITEM_MAGIC_DETECTED) && itemMagicChar(monst->carriedItem) && !canSeeMonster(monst));
	}
	...
	if (!playerCanSeeOrSense(x, y) && !(pmap[x][y].flags & (ITEM_DETECTED | HAS_PLAYER)) && (!monst || !monsterRevealed(monst)) && !monsterWithDetectedItem && (pmap[x][y].flags & (DISCOVERED | MAGIC_MAPPED)) && (pmap[x][y].flags & STABLE_MEMORY))
	{
	...
	else
	{
		...
		else if (((pmap[x][y].flags & HAS_ITEM) && (pmap[x][y].flags & ITEM_DETECTED) && itemMagicChar(theItem) && !playerCanSeeOrSense(x, y)) || monsterWithDetectedItem)
		{
		...
		else if ((pmap[x][y].flags & HAS_MONSTER) && (playerCanSeeOrSense(x, y) || ((monst->info.flags & MONST_IMMOBILE) && (pmap[x][y].flags & DISCOVERED))) && (!monsterIsHidden(monst, &player) || rogue.playbackOmniscience))
		{
			needDistinctness = true;
		...
		else if ((pmap[x][y].flags & HAS_ITEM) && !cellHasTerrainFlag(x, y, T_OBSTRUCTS_ITEMS) && (playerCanSeeOrSense(x, y) || ((pmap[x][y].flags & DISCOVERED) && !cellHasTerrainFlag(x, y, T_MOVES_ITEMS))))
		{
			needDistinctness = true;
			if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience)
			{
		...
		if (gasAugmentWeight && ((pmap[x][y].flags & DISCOVERED) || rogue.playbackOmniscience))
		{
			...
			if ((pmap[x][y].flags & HAS_MONSTER) && monst->status[STATUS_INVISIBLE] && playerCanSeeOrSense(x, y) && !monsterRevealed(monst) && !monsterHiddenBySubmersion(monst, &player))
			{
		...
		if (!(pmap[x][y].flags & (ANY_KIND_OF_VISIBLE | ITEM_DETECTED | HAS_PLAYER)) && !playerCanSeeOrSense(x, y) && (!monst || !monsterRevealed(monst)) && !monsterWithDetectedItem)
		{
	...
	if (((pmap[x][y].flags & ITEM_DETECTED) || monsterWithDetectedItem || (monst && monsterRevealed(monst))) && !playerCanSeeOrSense(x, y))
	{
		// do nothing
	}
...
void plotCharWithColor(uchar inputChar, short xLoc, short yLoc, const color *cellForeColor, const color *cellBackColor)
{
	...
	if (inputChar != displayBuffer[xLoc][yLoc].character || foreRed != displayBuffer[xLoc][yLoc].foreColorComponents[0] || foreGreen != displayBuffer[xLoc][yLoc].foreColorComponents[1] || foreBlue != displayBuffer[xLoc][yLoc].foreColorComponents[2] || backRed != displayBuffer[xLoc][yLoc].backColorComponents[0] || backGreen != displayBuffer[xLoc][yLoc].backColorComponents[1] || backBlue != displayBuffer[xLoc][yLoc].backColorComponents[2])
	{
				...
				unsigned long reqTileFlags, short frames, short maxRadius, short x, short y)
{
	...
	for (i = max(x - maxRadius, 0); i <= min(x + maxRadius, DCOLS - 1); i++)
	{
		...
		{
			if ((!reqTerrainFlags || cellHasTerrainFlag(reqTerrainFlags, i, j)) && (!reqTileFlags || (pmap[i][j].flags & reqTileFlags)) && (i - x) * (i - x) + (j - y) * (j - y) <= maxRadius * maxRadius)
			{
	...
	for (n = (invert ? stepCount - 1 : 0); (invert ? n >= 0 : n <= stepCount); n += (invert ? -1 : 1))
	{
		for (i = 0; i < COLS; i++)
		{
			for (j = 0; j < ROWS; j++)
			{
				...
				if (!invert && coordinatesAreInMap(windowToMapX(i), windowToMapY(j)) && distanceMap[windowToMapX(i)][windowToMapY(j)] >= 0 && distanceMap[windowToMapX(i)][windowToMapY(j)] < 30000)
				{
				...
				if (j < MESSAGE_LINES && i >= mapToWindowX(0) && i < mapToWindowX(strLenWithoutEscapes(displayedMessage[MESSAGE_LINES - j - 1])))
				{
...
void executeKeystroke(signed long keystroke, boolean controlKey, boolean shiftKey)
{
	...
	/* case VIEW_RECORDING_KEY:
		...
		confirmMessages();
		if ((rogue.playerTurnNumber < 50 || confirm("End this game and view a recording?", false)) && dialogChooseFile(path, RECORDING_SUFFIX, "View recording: "))
		{
			if (fileExists(path))
			{
	...
	/* case LOAD_SAVED_GAME_KEY:
		...
		confirmMessages();
		if ((rogue.playerTurnNumber < 50 || confirm("End this game and load a saved game?", false)) && dialogChooseFile(path, GAME_SUFFIX, "Open saved game: "))
		{
			if (fileExists(path))
			{
				strcpy(rogue.nextGamePath, path);
...
void stripShiftFromMovementKeystroke(signed long *keystroke)
{
	...
	if (newKey == LEFT_KEY || newKey == RIGHT_KEY || newKey == DOWN_KEY || newKey == UP_KEY || newKey == UPLEFT_KEY || newKey == UPRIGHT_KEY || newKey == DOWNLEFT_KEY || newKey == DOWNRIGHT_KEY)
	{
		*keystroke -= 'A' - 'a';
	}
}
...
void refreshSideBar(short focusX, short focusY, boolean focusedEntityMustGoFirst)
{
	...
	for (indirectVision = 0; indirectVision < 2; indirectVision++)
	{
		...
		do
		{
			...
			{
				if ((canDirectlySeeMonster(monst) || (indirectVision && (canSeeMonster(monst) || rogue.playbackOmniscience))) && !addedEntity[monst->xLoc][monst->yLoc] && !(monst->info.flags & MONST_NOT_LISTED_IN_SIDEBAR) && (px - monst->xLoc) * (px - monst->xLoc) + (py - monst->yLoc) * (py - monst->yLoc) < shortestDistance)
				{
		...
		do
		{
			...
			{
				if ((playerCanDirectlySee(theItem->xLoc, theItem->yLoc) || (indirectVision && (playerCanSeeOrSense(theItem->xLoc, theItem->yLoc) || rogue.playbackOmniscience))) && !addedEntity[theItem->xLoc][theItem->yLoc] && (px - theItem->xLoc) * (px - theItem->xLoc) + (py - theItem->yLoc) * (py - theItem->yLoc) < shortestDistance)
				{
		...
		for (k = 0; k < max(DROWS, DCOLS); k++)
		{
			for (i = px - k; i <= px + k; i++)
			{
				for (j = py - k; j <= py + k; j++)
				{
					if (coordinatesAreInMap(i, j) && (i == px - k || i == px + k || j == py - k || j == py + k) && !addedEntity[i][j] && (playerCanDirectlySee(i, j) || (indirectVision && (playerCanSeeOrSense(i, j) || rogue.playbackOmniscience))) && cellHasTMFlag(i, j, TM_LIST_IN_SIDEBAR) && displayEntityCount < ROWS - 1)
					{
...
void displayGrid(short **map)
{
	...
	for (i = 0; i < DCOLS; i++)
	{
		for (j = 0; j < DROWS; j++)
		{
			if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY | T_LAVA_INSTA_DEATH) || (map[i][j] == map[0][0]) || (i == player.xLoc && j == player.yLoc))
			{
				continue;
			}
...
short printMonsterInfo(creature *monst, short y, boolean dim, boolean highlight)
{
	...
	if (monst != &player && (!(monst->info.flags & MONST_INANIMATE) || monst->creatureState == MONSTER_ALLY))
	{
		...
		if (y < ROWS - 1)
		{
			...
			else if ((monst->info.flags & MONST_RESTRICTED_TO_LIQUID) && !cellHasTMFlag(monst->xLoc, monst->yLoc, TM_ALLOWS_SUBMERGING))
			{
