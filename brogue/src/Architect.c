...
short passableArcCount(short x, short y)
{
  ...
  for (dir = 0; dir < DIRECTION_COUNT; dir++)
  {
    ...
    if ((coordinatesAreInMap(newX, newY) && cellIsPassableOrDoor(newX, newY)) != (coordinatesAreInMap(oldX, oldY) && cellIsPassableOrDoor(oldX, oldY)))
    {
      arcCount++;
    }
  }
    ...
    for (j = 1; j < DROWS - 1; j++)
    {
      ...
      if (passMap[i][j] && !(pmap[i][j].flags & IN_LOOP))
      {
        ...
        for (dir = 0; dir < DIRECTION_COUNT; dir++)
        {
          ...
          newX = i + cDirs[dir][0];
          newY = j + cDirs[dir][1];
          if ((coordinatesAreInMap(newX, newY) && passMap[newX][newY]) != (coordinatesAreInMap(oldX, oldY) && passMap[oldX][oldY]))
          {
  ...
  for (i = 0; i < DCOLS * DROWS; i++)
  {
    ...
    if (!grid[x][y])
    {
      ...
      { // Try a horizontal door, and then a vertical door.
        ...
        if (coordinatesAreInMap(newX, newY) && coordinatesAreInMap(oppX, oppY) && grid[newX][newY] > 0 && grid[oppX][oppY] > 0)
...
boolean addTileToMachineInteriorAndIterate(char interior[DCOLS][DROWS], short startX, short startY)
{
  ...
  for (dir = 0; dir < 4 && goodSoFar; dir++)
  {
    ...
    if (coordinatesAreInMap(newX, newY))
    {
      if ((pmap[newX][newY].flags & HAS_ITEM) || ((pmap[newX][newY].flags & IS_IN_MACHINE) && !(pmap[newX][newY].flags & IS_GATE_SITE)))
      {
      ...
      if (!interior[newX][newY] && chokeMap[newX][newY] <= chokeMap[startX][startY] // don't have to worry about walls since they're all 30000
...
{
  if (blueprintCatalog[i].depthRange[0] > rogue.depthLevel || blueprintCatalog[i].depthRange[1] < rogue.depthLevel
                               ...
                               unsigned long bpFlags)
{
  ...
  { // Distance is calculated for walls too.
    ...
    for (dir = 0; dir < 4; dir++)
    {
      newX = x + nbDirs[dir][0];
      newY = y + nbDirs[dir][1];
      if (coordinatesAreInMap(newX, newY) && !cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) && distance > distanceMap[newX][newY] + 1)
      {
  ...
  { // If we're supposed to build in a wall...
    if (!interior[x][y] && (pmap[x][y].machineNumber == 0 || pmap[x][y].machineNumber == machineNumber) && cellHasTerrainFlag(x, y, T_OBSTRUCTS_PASSABILITY))
    { // ...and this location is a wall that's not already machined...
      for (dir = 0; dir < 4; dir++)
      {
        ...
        if (coordinatesAreInMap(newX, newY) // ...and it's next to an interior spot or permitted elsewhere and next to passable spot...
            && ((interior[newX][newY] && !(newX == originX && newY == originY)) || ((featureFlags & MF_BUILD_ANYWHERE_ON_LEVEL) && !cellHasTerrainFlag(newX, newY, T_PATHING_BLOCKER) && pmap[newX][newY].machineNumber == 0)))
        {
  ...
  {
    if ((featureFlags & MF_GENERATE_ITEM) && (cellHasTerrainFlag(x, y, T_OBSTRUCTS_ITEMS | T_PATHING_BLOCKER) || (pmap[x][y].flags & (IS_CHOKEPOINT | IN_LOOP | IS_IN_MACHINE))))
    {
      return false;
    }
    else
        ...
        if (cellHasTerrainFlag(i, j, T_PATHING_BLOCKER) && pmap[i][j].machineNumber == 0)
        {
          ...
          if (nbcount >= minimumInteriorNeighbors)
          {
            ...
            for (nbcount = dir = 0; dir < DIRECTION_COUNT; dir++)
            {
              newX = i + nbDirs[dir][0];
              newY = j + nbDirs[dir][1];
              if (!interior[newX][newY] && (!cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) || pmap[newX][newY].machineNumber != 0))
              {
                nbcount++;
                break;
              }
...
boolean fillInteriorForVestibuleMachine(char interior[DCOLS][DROWS], short bp, short originX, short originY)
{
  ...
  else if ((blueprintCatalog[bp].flags & BP_REQUIRE_BLOCKING) && levelIsDisconnectedWithBlockingMap(interior, true) < 100)
  {
    success = false;
  }
      ...
      for (j = 0; j < DROWS; j++)
      {
        if (interior[i][j] && !(pmap[i][j].flags & IS_GATE_SITE))
        {
          for (dir = 0; dir < DIRECTION_COUNT; dir++)
          {
            newX = i + nbDirs[dir][0];
            newY = j + nbDirs[dir][1];
            if (coordinatesAreInMap(newX, newY) && !interior[newX][newY] && !cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) && !(pmap[newX][newY].flags & IS_GATE_SITE) && !pmap[newX][newY].machineNumber && cellHasTerrainFlag(newX, newY, T_PATHING_BLOCKER))
            {
              for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++)
              {
                      ...
                      creature *parentSpawnedMonsters[MACHINES_BUFFER_LENGTH])
{
    ...
    if (blueprintCatalog[bp].flags & BP_ROOM)
    {
      ...
      if (chooseLocation)
      {
        ...
        for (i = 0; i < DCOLS; i++)
        {
          ...
          {
            if ((pmap[i][j].flags & IS_GATE_SITE) && !(pmap[i][j].flags & IS_IN_MACHINE) && chokeMap[i][j] >= blueprintCatalog[bp].roomSize[0] && chokeMap[i][j] <= blueprintCatalog[bp].roomSize[1])
            {
  ...
  for (feat = 0; feat < blueprintCatalog[bp].featureCount; feat++)
  {
    ...
    do
    { // If the MF_REPEAT_UNTIL_NO_PROGRESS flag is set, repeat until we fail to build the required number of instances.
      ...
      for (instance = 0; (generateEverywhere || instance < instanceCount) && qualifyingTileCount > 0;)
      {
        ...
        if (DFSucceeded && feature->terrain)
        {
          ...
          if (!(feature->flags & MF_PERMIT_BLOCKING) && ((tileCatalog[feature->terrain].flags & T_PATHING_BLOCKER) || (feature->flags & MF_TREAT_AS_BLOCKING)))
          {
        ...
        { // Proceed only if the terrain stuff for this instance succeeded.
          ...
          if ((feature->flags & MF_GENERATE_ITEM) || (adoptiveItem && (feature->flags & MF_ADOPT_ITEM) && (blueprintCatalog[bp].flags & BP_ADOPT_ITEM)))
          {
            ...
            else
            {
              ...
              failsafe = 1000;
              while ((theItem->flags & ITEM_CURSED) || ((feature->flags & MF_REQUIRE_GOOD_RUNIC) && (!(theItem->flags & ITEM_RUNIC))) // runic if requested
              ...
              { // don't want to duplicates of rings, staffs, etc.
                deleteItem(theItem);
                theItem = generateItem(feature->itemCategory, feature->itemKind);
                if (failsafe <= 0)
                {
...
void addMachines()
{
  ...
  if (rogue.depthLevel == AMULET_LEVEL)
  {
    ...
    {
      if (buildAMachine(MT_AMULET_AREA, -1, -1, 0UL, NULL, NULL, NULL))
      {
        break;
      }
    }
  ...
  machineCount = 0;
  while (rogue.depthLevel <= AMULET_LEVEL && (rogue.rewardRoomsGenerated + machineCount) * 4 + 2 < rogue.depthLevel * MACHINES_FACTOR)
  {
    ...
    if (gen->machine > 0 == buildAreaMachines)
    {
      ...
      for (i = 0; i < count; i++)
      {
        ...
        if (randomMatchingLocation(&x, &y, gen->requiredDungeonFoundationType, gen->requiredLiquidFoundationType, -1))
        {
          ...
          if (gen->terrain && tileCatalog[pmap[x][y].layers[gen->layer]].drawPriority >= tileCatalog[gen->terrain].drawPriority)
          {

            // Check connectivity.
            zeroOutGrid(grid);
            grid[x][y] = true;
            if (!(tileCatalog[gen->terrain].flags & T_PATHING_BLOCKER) || !levelIsDisconnectedWithBlockingMap(grid, false))
            {
  ...
  do
  {
         ...
         (reverse ? i-- : i++))
    {
           ...
           (reverse ? j-- : j++))
      {
        ...
        if (cellHasTerrainFlag(i, j, T_LAKE_PATHING_BLOCKER | T_OBSTRUCTS_PASSABILITY) && !cellHasTMFlag(i, j, TM_IS_SECRET) && !(pmap[i][j].flags & IMPREGNABLE))
        {

          subjectFlags = terrainFlags(i, j) & (T_LAKE_PATHING_BLOCKER | T_OBSTRUCTS_PASSABILITY);

          x = y = 0;
          if ((terrainFlags(i - 1, j) & T_LAKE_PATHING_BLOCKER & ~subjectFlags) && !cellHasTMFlag(i - 1, j, TM_IS_SECRET) && !cellHasTMFlag(i + 1, j, TM_IS_SECRET) && (terrainFlags(i - 1, j) & T_LAKE_PATHING_BLOCKER & ~subjectFlags) == (terrainFlags(i + 1, j) & T_LAKE_PATHING_BLOCKER & ~subjectFlags))
          {
            x = i + 1;
            y = j;
          }
          else if ((terrainFlags(i, j - 1) & T_LAKE_PATHING_BLOCKER & ~subjectFlags) && !cellHasTMFlag(i, j - 1, TM_IS_SECRET) && !cellHasTMFlag(i, j + 1, TM_IS_SECRET) && (terrainFlags(i, j - 1) & T_LAKE_PATHING_BLOCKER & ~subjectFlags) == (terrainFlags(i, j + 1) & T_LAKE_PATHING_BLOCKER & ~subjectFlags))
          {
            x = i;
            y = j + 1;
          }
  ...
  do
  {
    ...
    for (i = 0; i < DCOLS - 1; i++)
    {
      for (j = 0; j < DROWS - 1; j++)
      {
        for (k = 0; k <= 1; k++)
        {
          if (!(tileCatalog[pmap[i + k][j].layers[DUNGEON]].flags & T_OBSTRUCTS_PASSABILITY) && (tileCatalog[pmap[i + (1 - k)][j].layers[DUNGEON]].flags & T_OBSTRUCTS_PASSABILITY) && (tileCatalog[pmap[i + (1 - k)][j].layers[DUNGEON]].flags & T_OBSTRUCTS_DIAGONAL_MOVEMENT) && (tileCatalog[pmap[i + k][j + 1].layers[DUNGEON]].flags & T_OBSTRUCTS_PASSABILITY) && (tileCatalog[pmap[i + k][j + 1].layers[DUNGEON]].flags & T_OBSTRUCTS_DIAGONAL_MOVEMENT) && !(tileCatalog[pmap[i + (1 - k)][j + 1].layers[DUNGEON]].flags & T_OBSTRUCTS_PASSABILITY))
          {
...
void insertRoomAt(short **dungeonMap, short **roomMap, const short roomToDungeonX, const short roomToDungeonY, const short xRoom, const short yRoom)
{
  ...
  for (dir = 0; dir < 4; dir++)
  {
    ...
    if (coordinatesAreInMap(newX, newY) && roomMap[newX][newY] && coordinatesAreInMap(newX + roomToDungeonX, newY + roomToDungeonY) && dungeonMap[newX + roomToDungeonX][newY + roomToDungeonY] == 0)
    {
...
void attachHallwayTo(short **grid, short doorSites[4][2])
{
  ...
  {
    dir = dirs[i];
    if (doorSites[dir][0] != -1 && doorSites[dir][1] != -1 && coordinatesAreInMap(doorSites[dir][0] + nbDirs[dir][0] * HORIZONTAL_CORRIDOR_MAX_LENGTH, doorSites[dir][1] + nbDirs[dir][1] * VERTICAL_CORRIDOR_MAX_LENGTH))
    {
...
void attachRooms(short **grid, const dungeonProfile *theDP, short attempts, short maxRoomCount)
{
  ...
  for (roomsBuilt = roomsAttempted = 0; roomsBuilt < maxRoomCount && roomsAttempted < attempts; roomsAttempted++)
  {
    ...
    for (i = 0; i < DCOLS * DROWS; i++)
    {
      ...
      if (dir != NO_DIRECTION && doorSites[oppDir][0] != -1 && roomFitsAt(grid, roomMap, x - doorSites[oppDir][0], y - doorSites[oppDir][1]))
      {
    ...
    for (j = 0; j < DROWS; j++)
    {
      if (pmap[i][j].layers[DUNGEON] == GRANITE)
      {
        ...
        for (dir = 0; dir < (includingDiagonals ? 8 : 4) && !foundExposure; dir++)
        {
          x1 = i + nbDirs[dir][0];
          y1 = j + nbDirs[dir][1];
          if (coordinatesAreInMap(x1, y1) && (!cellHasTerrainFlag(x1, y1, T_OBSTRUCTS_VISION) || !cellHasTerrainFlag(x1, y1, T_OBSTRUCTS_PASSABILITY)))
          {
...
void lakeFloodFill(short x, short y, short **floodMap, short **grid, short **lakeMap, short dungeonToGridX, short dungeonToGridY)
{
  ...
  for (dir = 0; dir < 4; dir++)
  {
    newX = x + nbDirs[dir][0];
    newY = y + nbDirs[dir][1];
    if (coordinatesAreInMap(newX, newY) && !floodMap[newX][newY] && (!cellHasTerrainFlag(newX, newY, T_PATHING_BLOCKER) || cellHasTMFlag(newX, newY, TM_CONNECTS_LEVEL)) && !lakeMap[newX][newY] && (!coordinatesAreInMap(newX + dungeonToGridX, newY + dungeonToGridY) || !grid[newX + dungeonToGridX][newY + dungeonToGridY]))
    {
...
boolean lakeDisruptsPassability(short **grid, short **lakeMap, short dungeonToGridX, short dungeonToGridY)
{
  ...
  for (i = 0; i < DCOLS && x == -1; i++)
  {
    ...
    {
      if (!cellHasTerrainFlag(i, j, T_PATHING_BLOCKER) && !lakeMap[i][j] && (!coordinatesAreInMap(i + dungeonToGridX, j + dungeonToGridY) || !grid[i + dungeonToGridX][j + dungeonToGridY]))
      {
  ...
  for (i = 0; i < DCOLS && result == false; i++)
  {
    ...
    {
      if (!cellHasTerrainFlag(i, j, T_PATHING_BLOCKER) && !lakeMap[i][j] && !floodMap[i][j] && (!coordinatesAreInMap(i + dungeonToGridX, j + dungeonToGridY) || !grid[i + dungeonToGridX][j + dungeonToGridY]))
      {
    ...
    for (j = 0; j < DROWS; j++)
    {
      if (wreathMap[i][j])
      {
        for (k = i - wreathWidth; k <= i + wreathWidth; k++)
        {
          ...
          {
            if (coordinatesAreInMap(k, l) && pmap[k][l].layers[LIQUID] == NOTHING && (i - k) * (i - k) + (j - l) * (j - l) <= wreathWidth * wreathWidth)
            {
...
void finishDoors()
{
  ...
  for (i = 1; i < DCOLS - 1; i++)
  {
    for (j = 1; j < DROWS - 1; j++)
    {
      ...
      {
        if ((!cellHasTerrainFlag(i + 1, j, T_OBSTRUCTS_PASSABILITY) || !cellHasTerrainFlag(i - 1, j, T_OBSTRUCTS_PASSABILITY)) && (!cellHasTerrainFlag(i, j + 1, T_OBSTRUCTS_PASSABILITY) || !cellHasTerrainFlag(i, j - 1, T_OBSTRUCTS_PASSABILITY)))
        {
          // If there's passable terrain to the left or right, and there's passable terrain
          // above or below, then the door is orphaned and must be removed.
          pmap[i][j].layers[DUNGEON] = FLOOR;
        }
        else if ((cellHasTerrainFlag(i + 1, j, T_PATHING_BLOCKER) ? 1 : 0) + (cellHasTerrainFlag(i - 1, j, T_PATHING_BLOCKER) ? 1 : 0) + (cellHasTerrainFlag(i, j + 1, T_PATHING_BLOCKER) ? 1 : 0) + (cellHasTerrainFlag(i, j - 1, T_PATHING_BLOCKER) ? 1 : 0) >= 3)
        {
    ...
    for (j2 = 1; j2 < DROWS - 1; j2++)
    {
      ...
      if (!cellHasTerrainFlag(i, j, (T_CAN_BE_BRIDGED | T_PATHING_BLOCKER)) && !pmap[i][j].machineNumber)
      {
        ...
        if (k < DCOLS && (k - i > 3)                                                            // Can't have bridges shorter than 3 spaces.
            && foundExposure && !cellHasTerrainFlag(k, j, T_PATHING_BLOCKER | T_CAN_BE_BRIDGED) // Must end on an unobstructed land tile.
        ...
        foundExposure = false;
        for (k = j + 1;
             k < DROWS && !pmap[i][k].machineNumber && cellHasTerrainFlag(i, k, T_CAN_BE_BRIDGED) && !cellHasTMFlag(i, k, TM_IS_SECRET) && !cellHasTerrainFlag(i, k, T_OBSTRUCTS_PASSABILITY) && cellHasTerrainFlag(i - 1, k, (T_CAN_BE_BRIDGED | T_OBSTRUCTS_PASSABILITY)) && cellHasTerrainFlag(i + 1, k, (T_CAN_BE_BRIDGED | T_OBSTRUCTS_PASSABILITY));
             k++)
        {

          if (!cellHasTerrainFlag(i - 1, k, T_OBSTRUCTS_PASSABILITY) && !cellHasTerrainFlag(i + 1, k, T_OBSTRUCTS_PASSABILITY))
          {
            foundExposure = true;
          }
        }
        if (k < DROWS && (k - j > 3) && foundExposure && !cellHasTerrainFlag(i, k, T_PATHING_BLOCKER | T_CAN_BE_BRIDGED) && !pmap[i][k].machineNumber // Cannot end in a machine.
...
void updateMapToShore()
{
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      ...
      {
        costMap[i][j] = 1;
        rogue.mapToShore[i][j] = (cellHasTerrainFlag(i, j, T_LAVA_INSTA_DEATH | T_IS_DEEP_WATER | T_AUTO_DESCENT) && !cellHasTMFlag(i, j, TM_IS_SECRET)) ? 30000 : 0;
      }
    }
  }
...
void refreshWaypoint(short wpIndex)
{
  ...
  {
    if ((monst->creatureState == MONSTER_SLEEPING || (monst->info.flags & MONST_IMMOBILE) || (monst->bookkeepingFlags & MB_CAPTIVE)) && costMap[monst->xLoc][monst->yLoc] >= 0)
    {
...
short connectCell(short x, short y, short zoneLabel, char blockingMap[DCOLS][DROWS], char zoneMap[DCOLS][DROWS])
{
  ...
  for (dir = 0; dir < 4; dir++)
  {
    ...
    if (coordinatesAreInMap(newX, newY) && zoneMap[newX][newY] == 0 && (!blockingMap || !blockingMap[newX][newY]) && cellIsPassableOrDoor(newX, newY))
    {
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
          ...
          && (!blockedByOtherLayers || tileCatalog[pmap[i][j].layers[highestPriorityLayer(i, j, true)]].drawPriority >= tileCatalog[surfaceTileType].drawPriority))
      {
        ...
        if ((tileCatalog[surfaceTileType].flags & T_IS_FIRE) && !(tileCatalog[pmap[i][j].layers[layer]].flags & T_IS_FIRE))
        {
          pmap[i][j].flags |= CAUGHT_FIRE_THIS_TURN;
        }

        if ((tileCatalog[pmap[i][j].layers[layer]].flags & T_PATHING_BLOCKER) != (tileCatalog[surfaceTileType].flags & T_PATHING_BLOCKER))
        {
      ...
      for (j = 0; j < DROWS; j++)
      {
        if (spawnMap[i][j] == t - 1)
        {
          for (dir = 0; dir < 4; dir++)
          {
            x2 = i + nbDirs[dir][0];
            y2 = j + nbDirs[dir][1];
            if (coordinatesAreInMap(x2, y2) && (!requirePropTerrain || (propagationTerrain > 0 && cellHasTerrainType(x2, y2, propagationTerrain))) && (!cellHasTerrainFlag(x2, y2, T_OBSTRUCTS_SURFACE_EFFECTS) || (propagationTerrain > 0 && cellHasTerrainType(x2, y2, propagationTerrain))) && rand_percent(startProb))
            {
...
boolean spawnDungeonFeature(short x, short y, dungeonFeature *feat, boolean refreshCell, boolean abortIfBlocking)
{
  ...
  blocking = ((abortIfBlocking && !(feat->flags & DFF_PERMIT_BLOCKING) && ((tileCatalog[feat->tile].flags & (T_PATHING_BLOCKER)) || (feat->flags & DFF_TREAT_AS_BLOCKING))) ? true : false);
  ...
  if (feat->tile)
  {
    ...
    else
    {
      ...
      if (!blocking || !levelIsDisconnectedWithBlockingMap(blockingMap, false))
      {
        ...
        { // first, evacuate creatures if necessary, so that they do not re-trigger the tile.
  ...
  if (refreshCell && (tileCatalog[feat->tile].flags & (T_IS_FIRE | T_AUTO_DESCENT)) && cellHasTerrainFlag(player.xLoc, player.yLoc, (T_IS_FIRE | T_AUTO_DESCENT)))
  {
...
void initializeLevel()
{
  ...
  if (!levels[rogue.depthLevel - 1].visited)
  {
    ...
    for (dir = 0; dir < 4; dir++)
    {
      if (coordinatesAreInMap(upLoc[0] + nbDirs[dir][0], upLoc[1] + nbDirs[dir][1]) && !cellHasTerrainFlag(upLoc[0] + nbDirs[dir][0], upLoc[1] + nbDirs[dir][1], T_OBSTRUCTS_PASSABILITY))
      {
